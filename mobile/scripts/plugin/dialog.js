...
(function(G, D) {
    ...
    function Dialog(o) {
        ...
        this.config_adviceVal = o.adviceVal,
        ...
        this.config_footStyle = o.footStyle || {};
        if (isIE && isIE < 10) {
        ...
        this.config_subStyle = o.subStyle || {};
        this.config_canStyle = o.canStyle || {};
    ...
    Dialog.prototype.init = function() {
        ...
        if (!F$(this.config_id) && !this.config_msg && !this.config_title) {
            return false
        };
    ...
    Dialog.prototype.create = function() {
        ...
        switch(this.config_type){
            case "input":
                var html = "";
                html += "<div class='D_tit' style='color:#19273f; font-size: 16px; padding: 20px 0 10px 0'>" + this.config_title + "</div>";
                html += "<div class='D_alert'><input id='adviceInp' style='border:1px solid #ccc; font-size: 16px; width: 98%; height:40px; line-height:40px; text-align:center; background:#fff; margin: 0; padding: 0' type='number' value = '" + this.config_adviceVal + "' placeholder='请输入支付密码' /></div>";
                ...
                if (this.config_showButtons) {
                    ...
                    if (this.config_cancelButton) {
                        foot_html += '<input type="button" id="cancel" value="' + this.config_cancelButton + '" class="D_cancel" data-dialog-cancel />';
                    }
                    if (this.config_submitButton) {
                        foot_html += '<input type="button" id="submit" value="' + this.config_submitButton + '" class="D_submit" data-dialog-submit/>';
                    }
                ...
                this.foot = foot;
                this.fSub = foot.querySelector('#submit');
                this.fCan = foot.querySelector('#cancel')
                this.head = head;
            ...
            case "alert":
                ...
                this.bCon = body.querySelector('.D_alert');
                this.foot = foot;
                this.fSub = foot.querySelector('input');
                this.closeBtn = D.querySelector('.D_close');
                break;
            case "tips":
                content.className = "D_content";
    ...
    Dialog.prototype.setStyle = function() {
        ...
        if(this.body){
            ...
            setStyle(this.foot, this.config_footStyle);
            // setStyle(this.fBtn, {
            //     'border': 'none',
            //     'width': '100%',
            //     'height': '40px',
            ...
            //     'background': '#333'
            // });
            setStyle(this.fSub, this.config_subStyle);
            setStyle(this.fCan, this.config_canStyle);
            setStyle(this.bCon, {
                'padding': '20px 10px'
            });
            setStyle(this.bTips, {
    ...
    Dialog.prototype.animation = function() {
        ...
        if (this.config_animation) {
            ...
            for (var i = 0; i < events.length; i++) {
                this.content.addEventListener(events[i], function(){addFlash(that)}, false);
            }
        }
    };
    ...
    Dialog.prototype.close = function() {
        if (this.config_onClose && this.config_onClose(this) !== false) {
            this.remove();
        }
    };
    ...
    function addFlash(that) {
        that.addFlashFn && that.addFlashFn(that);
